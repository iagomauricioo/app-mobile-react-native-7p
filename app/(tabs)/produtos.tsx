...
export default function ProdutosScreen() {
  ...
  return (
    <View style={s.container}>
      <FlatList data={produtos} keyExtractor={(item) => item.id.toString()} renderItem={({ item }) => <ProdutoCard produto={item} onToggleDisponibilidade={() => handleToggle(item.id)} isToggling={togglingId === item.id} />} contentContainerStyle={{ padding: 16 }} ListEmptyComponent={<View style={[s.center, { paddingVertical: 48 }]}><Text style={{ color: '#A8A29E', fontSize: 16 }}>Nenhum produto encontrado</Text></View>} />
    </View>
  );
}
